define(function(require) {
	...
	var	commonMain=require('commonMain'),
		...
		table_body_focus_tag=$("#table_body_focus_tag"),
		add_category_btn=$("#add_category_btn"),
		add_goodtag_btn=$("#add_goodtag_btn"),
		add_focustag_btn=$("#add_focustag_btn"),
		perPagingCount=10,
	...
	var PagingCategory=new paging("#paging_category",MAXPAGING,function(currentPage){
		AJAXMY.send('/certification/category/list',{page:currentPage,size:perPagingCount,type:1},function(d){
	...
	table_body_category.on('click','.glyphicon-cancel',function(event){
		...
		AJAXMY.send('/certification/category/edit',{id:$(this).parent().attr("data-id")},function(data){
			...
			console.log(tthis.parent().parent());
	...
	table_body_category.on('click','.glyphicon-ok_add',function(event){
		...
		var tthis=$(this),
			...
			priority=tthis.parent().prev().text();	
		if(parseString.isEmpty(name)&&parseString.isNumber(priority)){
			AJAXMY.send('/certification/category/save',{type:1,name:name,priority:priority},function(data){
				tthis.prop('disabled',false);
				if(data['result']){					
	...
	table_body_category.on('click','.glyphicon-trash',function(event){
		...
		POPUPWINDOW.confirm("一匡后台","确认执行删除操作吗？再想想？",function(){
			AJAXMY.send('/certification/category/delete',{id:tthis.parent().attr("data-id")},function(d){
				if(d['result']){
...
/////////////////////////////////////擅长领域
	var PagingGoodTag=new paging("#paging_good_tag",MAXPAGING,function(currentPage){
		AJAXMY.send('/certification/tag/list',{page:currentPage,size:perPagingCount,type:1},function(d){
	...
	table_body_good_tag.on('click','.glyphicon-ok_add',function(event){
		...
		tthis.prop('disabled',true);	
		if(parseString.isEmpty(name)&&parseString.lessLength(name,5)){
			AJAXMY.send('/certification/tag/save',{type:1,name:name,status:domStatus.is(':checked')?1:0},function(data){
				tthis.prop('disabled',false);
				if(data['result']){
			...
			tthis.prop('disabled',false);
			POPUPWINDOW.alert('名称不能为空，不能超过5字符');
		}
	});
	//添加撤销操作
	...
	table_body_good_tag.on('click','.glyphicon-edit',function(event){
		var dom=$(this).parent();
		dom.siblings('.td_name').attr("contenteditable","true").css("background-color","white");
	...
	/////////////////////////////////////关注领域
	var PagingFocusTag=new paging("#paging_focus_tag",MAXPAGING,function(currentPage){
		AJAXMY.send('/certification/tag/list',{page:currentPage,size:perPagingCount,type:2},function(d){
			table_body_focus_tag.empty();
			var s='',status;
			$.each(d['result'],function(k,v){
				if(v['status']){
					status='<input class="checkbox" type="checkbox" checked>';
			...
			table_body_focus_tag.append(s);
			PagingFocusTag.refreshDom(d['pages']);
		});
	});
	//添加操作
	add_focustag_btn.click(function(){
		table_body_focus_tag.append('<tr class="table_new_row"><td contenteditable="true" class="td_name"></td><td class="td_status"><input type="checkbox"></td><td>/</td><td><button class="glyphicon-ok_add glyphicon onlyicon" title="提交"></button> <button class="glyphicon-cancel_add glyphicon onlyicon" title="取消"></button></td></tr>');
	});
	//添加提交操作
	table_body_focus_tag.on('click','.glyphicon-ok_add',function(event){
		var tthis=$(this),
			dom=tthis.parent(),
			domTr=dom.parent(),
			domName=dom.siblings('.td_name'),
		...
		tthis.prop('disabled',true);	
		if(parseString.isEmpty(name)&&parseString.lessLength(name,5)){
			AJAXMY.send('/certification/tag/save',{type:2,name:name,status:domStatus.is(':checked')?1:0},function(data){
				tthis.prop('disabled',false);
				if(data['result']){
					POPUPWINDOW.confirm('提示','添加成功，刷新当前列表才能编辑新增项，是否刷新',function(btn){
						btn.prop('disabled',false);
						//刷新列表
						PagingFocusTag.executePageFn(1);
						POPUPWINDOW.hide();
					},function(){
						disEditableCss(domTr,[domName]);
						domStatus.prop("disabled",true);
	...
	//添加撤销操作
	table_body_focus_tag.on('click','.glyphicon-cancel_add',function(event){
		var domTr=$(this).parent().parent();
		domTr.remove();
	});
	//编辑操作
	table_body_focus_tag.on('click','.glyphicon-edit',function(event){
		var dom=$(this).parent();
	...
	//编辑保存操作
	table_body_focus_tag.on('click','.glyphicon-ok',function(event){
		$(this).prop('disabled',true);
		var tthis=$(this),
			dom=tthis.parent(),
			domTr=dom.parent(),
	...
	//编辑撤销操作
	table_body_focus_tag.on('click','.glyphicon-cancel',function(event){
		var tthis=$(this),
			dom=tthis.parent(),
			domTr=dom.parent();
		tthis.prop('disabled',true);	
	...
	//删除操作
	table_body_focus_tag.on('click','.glyphicon-trash',function(event){
	...
	//选中事件
	table_body_focus_tag.on('change','.checkbox',function(event){
		var tthis=$(this),
			status=tthis.is(':checked')?1:0;
		tthis.prop('disabled',true);		
		AJAXMY.send('/certification/tag/status',{id:tthis.parent().parent().attr("data-id"),status:status},function(data){
