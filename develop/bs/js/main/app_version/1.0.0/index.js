	...
	function request_list(getPaging,otherData){
		...
		AJAXMY.send('/version/list',data,function(d){
			...
			$.each(d['result'],function(k,v){
				...
				s+='<tr data-id="'+v['id']+'"><td>'+v['id']+'</td><td>'+androidIos[0]+'</td><td>'+v['versionDesc']+'</td><td>'+v['versionNum']+'</td><td><input type="checkbox" '+checkStatus[v['forced']]+'></td><td>'+androidIos[1]+'</td><td>'+androidIos[2]+'</td><td><input type="checkbox" '+checkStatus[v['status']]+'></td><td><div class="btn_group"><a href="version_edit.html?id='+v['id']+'" class="button btn_xs">编辑</a><button class="button btn_xs deleteversion">删除</button></div></td></tr>';
			});
			table_body.html(s);
