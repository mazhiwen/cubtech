define(function(require,exports,module) {
	...
	popUpWindow.prototype.confirm=function(title,content,yesFunction,noFunction){
		var tthis=this;
		this.initPUW();
		this.puwb.append('<div id="popupwin" class="popupwin_confirm"><div>'+title+'<span style="display:none;"></span></div><div>'+content+'</div><div><button>取消</button><button>确认</button></div></div>');
	...
	popUpWindow.prototype.alert=function(content,yesFunction){
		...
		*/
		if(this.puwb.css("display")=='none'){
			this.initPUW();
			this.puwb.append('<div id="popupwin" class="popupwin_alert"><div>提示<span style="display:none;"></span></div><div>'+content+'</div><div><button>确认</button></div></div>');
